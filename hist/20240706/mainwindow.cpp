    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->label_2d->setMinimumSize(qApp->primaryScreen()->size()-QSize{300,100});
    ui->label_2d->setMaximumSize(qApp->primaryScreen()->size()-QSize{300,100});
}
...
SPMSG MainWindow::InitUI()
{
    ...
    GLOG.setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Minimum);
    GLOG.setMinimumWidth(qApp->primaryScreen()->size().width()-200);
    ...
    GLOG << RegisterImgProc<CLog>(GLOG);
    //------------------------------------------------------------
    ...
    on_pushButton_opendir_clicked(Cfg("pushButton_opendir"));
    on_pushButton_savedir_clicked(Cfg("pushButton_savedir"));
    on_listWidget_files_currentTextChanged(Cfg("listWidget_files"));
    //---------------------------WAY----------------------------
    ...
    ui->comboBox_way->setCurrentText(Cfg("comboBox_way"));
    return _ok;
}
...
SPMSG MainWindow::slotProcess2D()
{
    ...
    m_spIMG = MatToIMG(dsc);
    return ui->label_2d->SetImg(m_spIMG);
}
...
SPMSG MainWindow::AddProc(CImgProc* pProc)
{
...
void MainWindow::on_pushButton_procadd_clicked()
{
    ...
    if (!sel->ok()){ return; }
    auto proc = CImgProc::NewProc(sel->body);
    if (!proc){ return; }
    AddProc(proc);
    GLOG << slotProcess2D();
}
...
void MainWindow::on_pushButton_saveimg_clicked()
{
    ...
    if (ui->radioButton_newsize->isChecked()){
        auto dsc = m_spIMG->scaled({ui->spinBox_save_imgw->value(),ui->spinBox_save_imgh->value()});
        dsc.save(save_file.absoluteFilePath());
    } else {
        m_spIMG->save(save_file.absoluteFilePath());
    }
